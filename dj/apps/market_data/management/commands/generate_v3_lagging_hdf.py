...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        df = add_forecasts_to_df(df, live=False, shift=120)
        for column_name in (
        ...
        save_output(output_df_v2_lagged, "v3_lagging_A" + kwargs["hdf_filename"])
        output_df_v2_lagged = self.B_transform(df.copy(), symbols_kline_attrs)
        save_output(output_df_v2_lagged, "v3_lagging_B" + kwargs["hdf_filename"])
        output_df_v2_lagged = self.C_transform(df.copy(), symbols_kline_attrs)
        save_output(output_df_v2_lagged, "v3_lagging_C" + kwargs["hdf_filename"])
