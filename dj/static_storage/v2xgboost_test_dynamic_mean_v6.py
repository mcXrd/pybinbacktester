...
def simulate(shift_in_hours, path_to_hdf, days, maxdays, coin, mean_const=1):
    ...
    del df_orig["trade_in_15m_usdtfutures_{}USDT_close_price".format(coin)]

    shift_in_hours_maxdays = maxdays * 24
    shift_in_hours_days = days * 24
...
for shift_in_hours in tqdm.tqdm(range(1, 20 * 24, int(TEST_SIZE_DAYS * 24))):
    ...
    bank_c, trading_hours, skipped_ratio, hours_in_test, model, sides_c = simulate(
        shift, c, 5, "ETH", mean_const=mean_const
    )
    ...
    bank_f, trading_hours, skipped_ratio, hours_in_test, model, sides_f = simulate(
        shift, c, 11, "ETH", mean_const=mean_const
    )
    ...
    bank_c2, trading_hours, skipped_ratio, hours_in_test, model, sides_c2 = simulate(
        shift, c2, 5, "ADA", mean_const=mean_const
    )
    ...
    bank_f2, trading_hours, skipped_ratio, hours_in_test, model, sides_f2 = simulate(
        shift, c2, 11, "ADA", mean_const=mean_const
    )
    ...
    bank_cx, trading_hours, skipped_ratio, hours_in_test, model, sides_cx = simulate(
        shift, c, 5, "ETH", mean_const=mean_const
    )
    ...
    bank_fx, trading_hours, skipped_ratio, hours_in_test, model, sides_fx = simulate(
        shift, c, 11, "ETH", mean_const=mean_const
    )
    ...
    bank_c2x, trading_hours, skipped_ratio, hours_in_test, model, sides_c2x = simulate(
        shift, c2, 5, "ADA", mean_const=mean_const
    )
    ...
    bank_f2x, trading_hours, skipped_ratio, hours_in_test, model, sides_f2x = simulate(
        shift, c2, 11, "ADA", mean_const=mean_const
    )
    ...
    if file_to_pick_actual_result == "C":
        initial_bank, sides = bank_c, sides_c
    ...
    if file_to_pick_actual_result == "F":
        initial_bank, sides = bank_f, sides_f
    ...
    if file_to_pick_actual_result == "C2":
        initial_bank, sides = bank_c2, sides_c2
    ...
    if file_to_pick_actual_result == "F2":
        initial_bank, sides = bank_f2, sides_f2
    ...
    if file_to_pick_actual_result == "Cx":
        initial_bank, sides = bank_cx, sides_cx
    ...
    if file_to_pick_actual_result == "Fx":
        initial_bank, sides = bank_fx, sides_fx
    ...
    if file_to_pick_actual_result == "C2x":
        initial_bank, sides = bank_c2x, sides_c2x
    ...
    if file_to_pick_actual_result == "F2x":
        initial_bank, sides = bank_f2x, sides_f2x
